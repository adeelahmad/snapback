...
| Platforms | Linux and macOS (Linux first; macOS is a separately proven follow-up, section 22.1); amd64 and arm64 required, plus smaller Linux targets (e.g. MIPS for OpenWrt) as cross-compiled builds that are smoke-tested under QEMU on every CI run; a FUSE mount is not tested on that hardware. Windows excluded. |
...
| Channel | Requirement |
| --- | --- |
| Release binaries | Linux: static Go binaries (`CGO_ENABLED=0`, called static only when verified with `ldd`/`file`) for linux/amd64 and linux/arm64, in the first public release. macOS: self-contained Go application executables for darwin/amd64 and darwin/arm64, with linkage and runtime requirements to be verified, shipped with the macOS follow-up. Additional cross-compiled Linux targets (linux/arm v7, linux/mips, linux/mipsle) are smoke-tested under QEMU on every CI run — `snapback version` and the pure-Go unit tests (`internal/config`, `setup`, `cli`, `resolver`, `aliases`, `errcode`, `version`) execute emulated — while a FUSE mount is not tested on that hardware. SHA-256 checksums and Sigstore/cosign or GPG signatures on every artifact. |
